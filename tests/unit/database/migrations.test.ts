...
describe("database migrations workflow", () => {
  ...
  it("ships ordered 001–039 migration files", () => {
    expect(files).toEqual([
      ...
      "038_public_image_and_face_modules.sql",
      "039_seo_knowledge_cms.sql",
    ]);
  });
  ...
  it("adds vollmacht reject + staff message in 033", () => {
    const sql = readFileSync(
      path.join(dir, "033_order_desk_vollmacht_reject.sql"),
      "utf8"
    );
    expect(sql).toContain("reject_reason");
    expect(sql).toContain("staff_message");
  ...
  it("adds admin finanzen tables in 017", () => {
    const sql = readFileSync(path.join(dir, "017_admin_finanzen.sql"), "utf8");
    expect(sql).toContain("CREATE TABLE IF NOT EXISTS `api_cost_settings`");
