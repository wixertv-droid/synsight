...
export async function ensureHitActionsSchema(): Promise<boolean> {
  if (!ensurePromise) {
    ensurePromise = runEnsure().catch((error) => {
      ensurePromise = null;
      console.error("[ensureHitActionsSchema] failed", error);
      return false;
    });
  }
...
/** @deprecated */
export const ensureUsernameActionsSchema = ensureHitActionsSchema;
...
async function runEnsure(): Promise<boolean> {
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS username_hit_actions (
      ...
      action ENUM('ignored','self','ordered','resolved') NOT NULL,
      created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
      updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
      PRIMARY KEY (id),
      UNIQUE KEY username_hit_actions_user_mod_fp (user_id, source_module, hit_fingerprint),
  ...
  try {
    await db.execute(sql`
      ALTER TABLE username_hit_actions
      ADD COLUMN source_module VARCHAR(64) NOT NULL DEFAULT 'username_intelligence' AFTER analysis_id
    `);
  } catch {
    /* exists */
  }
  ...
  try {
    await db.execute(sql`
      ALTER TABLE username_hit_actions
      MODIFY COLUMN action ENUM('ignored','self','ordered','resolved') NOT NULL
    `);
  } catch {
    /* ok */
  }
