import { NextResponse } from "next/server";
import { apiError, apiSuccess } from "@/lib/api/response";
import {
  processAndStoreProfileImage,
  removeStoredProfileImage,
} from "@/lib/media/image-pipeline";
import { persistProcessedProfileImage } from "@/lib/services/identity-service";
import { resolveMobileUploadSession } from "@/lib/services/mobile-image-upload-service";
import {
  IMAGE_UPLOAD_RATE_LIMIT,
  rateLimitHeaders,
  recordRateLimitAttempt,
} from "@/lib/security/rate-limit";
import { getClientIp, validateMutationOrigin } from "@/lib/security/request";

const IMAGE_TYPES = new Set([
  "front",
  "left_profile",
  "right_profile",
  "angled",
]);

export async function handleMobileProfileImageUpload(request: Request) {
  const csrfError = validateMutationOrigin(request);
  if (csrfError) return csrfError;

  const ipAddress = getClientIp(request);
  const rateLimit = recordRateLimitAttempt(
    `mobile-image-upload:${ipAddress}`,
    IMAGE_UPLOAD_RATE_LIMIT
  );
  if (!rateLimit.allowed) {
    return NextResponse.json(
      apiError(
        "RATE_LIMITED",
        "Zu viele Bild-Uploads. Bitte versuchen Sie es später erneut."
      ),
      { status: 429, headers: rateLimitHeaders(rateLimit) }
    );
  }

  const form = await request.formData().catch(() => null);
  if (!form) {
    return NextResponse.json(
      apiError("VALIDATION_ERROR", "Ungültige Bildanfrage."),
      { status: 400, headers: rateLimitHeaders(rateLimit) }
    );
  }

  const token = String(form.get("token") ?? "");
  const imageType = String(form.get("imageType") ?? "");
  const file = form.get("file");

  const session = await resolveMobileUploadSession(token);
  if (!session) {
    return NextResponse.json(
      apiError(
        "INVALID_TOKEN",
        "Dieser Handy-Upload-Link ist ungültig oder abgelaufen. Bitte erzeugen Sie einen neuen QR-Code."
      ),
      { status: 401, headers: rateLimitHeaders(rateLimit) }
    );
  }

  if (!IMAGE_TYPES.has(imageType) || !(file instanceof File)) {
    return NextResponse.json(
      apiError(
        "VALIDATION_ERROR",
        "Bildtyp oder Datei fehlt beziehungsweise ist ungültig."
      ),
      { status: 400, headers: rateLimitHeaders(rateLimit) }
    );
  }

  const typedImageType = imageType as
    "front" | "left_profile" | "right_profile" | "angled";

  let processed = null;
  try {
    processed = await processAndStoreProfileImage({
      userId: session.userId,
      imageType: typedImageType,
      fileName: file.name,
      mimeType: file.type,
      bytes: Buffer.from(await file.arrayBuffer()),
    });
    const persisted = await persistProcessedProfileImage(
      session.userId,
      processed
    );
    const refreshed = await resolveMobileUploadSession(token);
    return NextResponse.json(
      apiSuccess({
        image: persisted,
        slots: refreshed?.slots ?? session.slots,
      }),
      { status: 201, headers: rateLimitHeaders(rateLimit) }
    );
  } catch (error) {
    if (processed) {
      await removeStoredProfileImage(
        session.userId,
        processed.storagePath
      ).catch(() => undefined);
    }
    return NextResponse.json(
      apiError(
        "IMAGE_PROCESSING_FAILED",
        error instanceof Error
          ? error.message
          : "Das Bild konnte nicht verarbeitet werden."
      ),
      { status: 400, headers: rateLimitHeaders(rateLimit) }
    );
  }
}
