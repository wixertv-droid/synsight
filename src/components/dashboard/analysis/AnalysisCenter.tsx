...
import { ANALYSIS_CENTER_RAIL } from "@/lib/dashboard/page-rails";
...
export default async function AnalysisCenter() {
  ...
  const railSections = ANALYSIS_CENTER_RAIL.filter((section) => {
    if (section.id === "tier-premium") return premium.length > 0;
    if (section.id === "tier-quick") {
      return byTier.some((group) => group.tier === "quick");
    }
    if (section.id === "tier-advanced") {
      return byTier.some((group) => group.tier === "advanced");
    }
    return true;
  });
        ...
        {modules.length === 0 ? (
          <section className="glass-strong hardware-panel rounded-[1.4rem] border border-amber-300/20 p-6 md:p-8">
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/40">
              Sobald ein Administrator Analysen in der Preisverwaltung
              aktiviert, erscheinen sie hier automatisch mit Preis und
              Beschreibung.
            </p>
          </section>
        ) : (
          <>
            <section
              id="analysis-intro"
              className="glass-strong hardware-panel mb-8 scroll-mt-24 rounded-[1.4rem] border border-cyber-cyan/20 p-5 md:p-6"
            >
                ...
                <div className="max-w-3xl">
                  ...
                  <ol className="mt-4 grid gap-2 text-[12px] text-white/45 sm:grid-cols-3">
                    <li className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                      ...
                      <p className="mt-1">
                        Profilangaben ergänzen (freiwillig)
                      </p>
                    </li>
                    <li className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                      <span className="font-mono text-[8px] text-cyber-cyan/50">
